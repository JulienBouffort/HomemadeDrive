                            ...
                            onclick: move |_| {
                                spawn(async move {
                                    ...
                                    let pending_photos: Vec<(String, Vec<u8>)> = photos
                                        ...
                                        .map(|p| (p.name.clone(), p.bytes.as_ref().unwrap().clone()))
                                        .collect();

                                    println!("📸 {} photo(s) prête(s) à l'envoi en parallèle", pending_photos.len());

                                    // 2. On passe toutes les photos concernées en "Uploading" avant de lancer les envois
                                    for (name, _) in &pending_photos {
                                    ...
                                    // 4. On traite les résultats une fois tous les uploads terminés
